  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Hero Banner Manager</h1>
          <p className="text-sm text-gray-500">Manage main section banners for all website pages</p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 border border-gray-100 p-4 rounded-xl mb-4">
            <div className="space-y-2">
              ...
              <select
                ...
                className="w-full px-4 py-2 border border-blue-900/20 rounded-lg focus:ring-2 focus:ring-blue-900/40 bg-white shadow-sm"
              >
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-6 border-b border-gray-50 flex justify-between items-center">
          <h2 className="text-lg font-semibold text-gray-800">Current Hero Banners</h2>
          <span className="text-xs font-bold text-blue-900 bg-blue-50 px-3 py-1 rounded-full border border-blue-100 uppercase">
          ...
          <table className="w-full text-left">
            <thead className="bg-[#959190]/10">
              <tr>
                ...
                <th className="px-6 py-4 text-xs font-bold text-gray-600 uppercase tracking-wider">Page Path</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {banners.map((banner, index) => (
                <tr key={banner._id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="text-sm font-semibold text-gray-800">{banner.titleHighlight1} {banner.titleHighlight2}</div>
                    <div className="text-[10px] text-gray-500 truncate max-w-xs">{banner.subtitle}</div>
                  </td>
                  <td className="px-6 py-4">
                     <code className="text-[10px] text-amber-800 bg-amber-100 rounded px-2 py-0.5 font-bold border border-amber-200">
                       {banner.pagePath || '/'}
                     </code>
                  </td>
                  <td className="px-6 py-4 text-center">
                    {banner.isActive ? (
                      ...
                      <span className="text-[10px] font-bold px-2 py-1 rounded-full bg-red-100 text-red-700 uppercase">Inactive</span>
                    )}
                  </td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-2">
                       <button onClick={() => openViewModal(banner)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-all" title="View Full Details">
                        <FiEye size={18} />
                      </button>
                      <button onClick={() => handleEdit(banner)} className="p-2 bg-green-50 text-green-600 hover:bg-green-100 rounded-lg transition-all border border-green-200" title="Edit">
                        <FiEdit2 size={16} />
                      </button>
                      <button onClick={() => handleDelete(banner._id)} className="p-2 bg-red-50 text-red-600 hover:bg-red-100 rounded-lg transition-all border border-red-200" title="Delete">
                        <FiTrash2 size={16} />
                      </button>
                    </div>
                  </td>
              ...
              {banners.length === 0 && !bannersLoading && (
                <tr>
                  <td colSpan="6" className="px-6 py-12 text-center text-gray-400 italic">No hero banners found. Add your first placeholder above.</td>
                </tr>
              )}
            </tbody>
          </table>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
            <div className="bg-gradient-to-r from-blue-900 to-blue-800 px-6 py-4 flex justify-between items-center text-white">
              <div className="flex items-center gap-3">
                <FiEye size={20} />
                <h3 className="font-bold text-lg">Hero Banner Details</h3>
              </div>
            ...
            <div className="p-6 max-h-[75vh] overflow-y-auto custom-scrollbar">
              {viewBanner.imageUrl && (
                <div className="mb-6 rounded-xl overflow-hidden shadow-md border border-gray-100 bg-gray-100">
                  <img src={`${BACKEND_URL}${viewBanner.imageUrl}`} alt="Banner" className="w-full h-56 object-cover object-center" />
                </div>
              )}

              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Badge Text</p>
                  <p className="font-bold text-gray-800">{viewBanner.badge}</p>
                </div>
                <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Status</p>
                  {viewBanner.isActive ? (
                    <span className="text-xs font-bold px-3 py-1 rounded-full bg-green-100 text-green-700 border border-green-200">ACTIVE</span>
                  ) : (
                    <span className="text-xs font-bold px-3 py-1 rounded-full bg-red-100 text-red-700 border border-red-200">INACTIVE</span>
                  )}
                </div>
                <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Assigned Page</p>
                  <code className="bg-blue-50 text-blue-900 px-2 py-0.5 rounded text-sm font-bold">{viewBanner.pagePath || '/'}</code>
                </div>
                <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                  <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Button Call-to-Action</p>
                  <p className="font-bold text-blue-900 text-sm italic">{viewBanner.linkText || 'None'}</p>
                </div>
              </div>

              <div className="bg-gray-50 p-5 rounded-xl border border-gray-100 mb-6">
                <h4 className="text-[11px] font-bold text-blue-900 uppercase tracking-widest mb-4 border-b border-blue-900/10 pb-2">Full Title Composition</h4>
                <div className="flex flex-wrap gap-2 text-lg">
                  <span className="text-gray-400">{viewBanner.titleHighlight1}</span>
                  <span className="font-bold text-red-600">{viewBanner.titleHighlight2}</span>
                  <span className="font-bold text-blue-900">{viewBanner.titleHighlight3}</span>
                  <span className="text-gray-800">{viewBanner.titleEnd}</span>
                </div>
              </div>

              <div className="bg-gray-50 p-5 rounded-xl border border-gray-100">
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2">Subtitle Content</p>
                <p className="text-sm text-gray-700 leading-relaxed font-medium italic">"{viewBanner.subtitle}"</p>
              </div>
            </div>
            ...
            <div className="bg-gray-50 px-6 py-4 flex justify-end gap-3 border-t border-gray-100">
              <button
                ...
                className="px-6 py-2 bg-blue-900 text-white rounded-xl font-bold text-sm shadow-md hover:brightness-110 transition-all flex items-center gap-2"
              >
                <FiEdit2 size={14} /> Edit Banner
              </button>
              <button onClick={() => setIsViewModalOpen(false)} className="px-6 py-2 bg-white text-gray-600 border border-gray-200 rounded-xl font-bold text-sm hover:bg-gray-50 transition-all">
                Close
              </button>
            </div>
          </div>

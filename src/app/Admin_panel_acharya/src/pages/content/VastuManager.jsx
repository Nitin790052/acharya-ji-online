...
import {
    useGetVastuSettingsQuery, useUpdateVastuSettingsMutation,
    useGetAllVastuServicesQuery, useCreateVastuServiceMutation,
    useUpdateVastuServiceMutation, useDeleteVastuServiceMutation,
    useSeedVastuServicesMutation,
} from '../../../../../services/vastuApi';
import { BACKEND_URL } from '../../../../../config/apiConfig';

const iconOptions = ['Home', 'Building2', 'Gem', 'Circle', 'Leaf', 'Shield', 'Sparkles', 'Star'];

const emptyForm = { title: '', description: '', features: '', benefits: '', price: '', iconName: 'Home', gradient: 'from-saffron to-orange-600', popular: false, isActive: true, order: '' };

export default function VastuManager() {
    ...
    const [deleteService] = useDeleteVastuServiceMutation();
    const [seedServices] = useSeedVastuServicesMutation();
    ...
    const handleEdit = (s) => {
        setForm({ title: s.title, description: s.description, features: (s.features || []).join(', '), benefits: (s.benefits || []).join(', '), price: s.price, iconName: s.iconName || 'Home', gradient: s.gradient || 'from-saffron to-orange-600', popular: s.popular || false, isActive: s.isActive !== false, order: s.order || '' });
    ...
    const handleSettingsSave = async () => {
        try { await updateSettings(settingsForm).unwrap(); toast.success('Settings saved!'); } catch { toast.error('Failed'); }
    };
        ...
        <div className="p-6 max-w-7xl mx-auto">
            ...
            <div className="bg-white rounded-xl shadow-sm border p-5 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div><label className={labelCls}>Badge Text</label><input className={inputCls} value={settingsForm.badge} onChange={e => setSettingsForm(p => ({ ...p, badge: e.target.value }))} placeholder="e.g. Transform Your Life" /></div>
                    <div><label className={labelCls}>Section Title</label><input className={inputCls} value={settingsForm.title} onChange={e => setSettingsForm(p => ({ ...p, title: e.target.value }))} placeholder="e.g. Vastu, Remedies & Healing" /></div>
            ...
            <div className="bg-white rounded-xl shadow-sm border p-5 mb-6">
                ...
                <form onSubmit={handleSubmit}>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        <div><label className={labelCls}>Title *</label><input className={inputCls} value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} required placeholder="e.g. Home Vastu" /></div>
                        <div><label className={labelCls}>Price *</label><input className={inputCls} value={form.price} onChange={e => setForm(p => ({ ...p, price: e.target.value }))} required placeholder="e.g. ₹2,999" /></div>
                        ...
                        <div className="md:col-span-1"><label className={labelCls}>Features (comma-separated)</label><input className={inputCls} value={form.features} onChange={e => setForm(p => ({ ...p, features: e.target.value }))} placeholder="e.g. Direction Analysis, Room Placement" /></div>
                        <div className="md:col-span-1"><label className={labelCls}>Benefits (comma-separated)</label><input className={inputCls} value={form.benefits} onChange={e => setForm(p => ({ ...p, benefits: e.target.value }))} placeholder="e.g. Harmony, Prosperity, Health" /></div>
                        <div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead className="bg-gray-50 border-b">
                                <tr>
                                    {['#', 'Order', 'Image', 'Title & Description', 'Price', 'Benefits', 'Active', 'Popular', 'Actions'].map(h => (
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {services.map((s, i) => (
                                    <tr key={s._id} className="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td className="px-4 py-3 text-xs text-gray-500">{(s.benefits || []).join(', ')}</td>
                                        <td className="px-4 py-3">
                    ...
                    <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="p-5">
                            ...
                            {viewItem.benefits?.length > 0 && (
                                <div className="mb-3"><span className="text-xs font-bold text-gray-500 uppercase block mb-1">Benefits:</span>
                                    <div className="flex flex-wrap gap-1.5">{viewItem.benefits.map((b, i) => <span key={i} className="bg-amber-50 text-amber-700 px-2 py-0.5 rounded-full text-xs">{b}</span>)}</div>
                                </div>
                            )}

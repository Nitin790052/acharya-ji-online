...
const Login = () => {
            ...
            <div className="grid grid-cols-2 gap-2">
              {roles.map((v) => {
                ...
                return (
                  <button
                    key={v.value}
                    onClick={() => setRole(v.value)}
                    className={`flex flex-col items-center p-2.5 rounded-lg border transition-all duration-200 ${
                      role === v.value
                        ? "border-orange-500 bg-gradient-to-br from-orange-50 to-amber-50 shadow-sm"
  ...
  if (role === "vendor") {
    ...
    login({
      ...vendor,       // id, name, category, mobile
      role: "vendor",  // VendorRoute ke liye
      isLoggedIn: true,
    });

...
export default function VedicScholar({ commonData, onBack, onSubmit }) {
  ...
  const validateForm = () => {
    ...
    if (formData.shastraExpertise.length === 0) {
      newErrors.shastraExpertise = 'Please select at least one shastra expertise';
    }

    if (!formData.teachingExperience || Number(formData.teachingExperience) < 0) {
      newErrors.teachingExperience = 'Please enter valid teaching experience';
    }

    if (formData.teachingModes.length === 0) {
      newErrors.teachingModes = 'Please select at least one teaching mode';
    }

    if (!formData.affiliatedInstitution.trim()) {
      newErrors.affiliatedInstitution = 'Affiliated institution is required';
    }
    ...
    if (formData.researchArea.trim().length < 50) {
      newErrors.researchArea = 'Please provide at least 50 characters';
    }
          ...
          <div className="relative z-10  p-3 sm:p-4 md:py-6 md:px-4 lg:py-5 lg:px-7">
            ...
            <div className="text-center mb-4 sm:mb-5 md:mb-6">
              ...
              <div className="inline-flex items-center gap-1.5 sm:gap-2 px-2.5 py-1 sm:px-3.5 sm:py-1.5 bg-gradient-to-r from-amber-100 to-orange-100 rounded-full border border-amber-200">
                <BookOpen size={12} className="sm:w-4 sm:h-4 text-amber-600" />
                <p className="text-[10px] sm:text-xs font-medium text-amber-700 whitespace-nowrap">
                  Step 2 of 2 - Scholarly Details
                </p>
              </div>
              ...
              <div className="space-y-3 sm:space-y-3.5">
                ...
                <div className="mb-3 sm:mb-3.5">
                  ...
                  <div className="grid grid-cols-2 gap-2 sm:gap-2.5">
                    {shastraOptions.map(({ value, label }) => (
                      <label
                        key={value}
                      ...
                      >
                        <input
                          type="checkbox"
                          className="sr-only"
                          checked={formData.shastraExpertise.includes(value)}
                          onChange={() => handleCheckboxChange('shastraExpertise', value)}
                          disabled={isSubmitting}
                        />
                ...
                <div className="mb-3 sm:mb-3.5">
                  <label className="block text-xs sm:text-sm font-medium text-gray-800 mb-1.5 sm:mb-1.5 ml-1">
                    <span className="flex items-center gap-1.5 sm:gap-2">
                      <div className="w-4 h-4 sm:w-5 sm:h-5 rounded-md bg-gradient-to-br from-orange-500 to-amber-500 flex items-center justify-center">
                        <Book size={10} className="sm:w-3 sm:h-3 text-white" />
                      </div>
                  ...
                  <div className="relative group">
                    <textarea
                      ...
                      onChange={handleInputChange}
                      rows="3"
                      placeholder="List your books, papers, or scholarly articles..."
                      className="w-full bg-white/90 border-2 border-gray-300/80 rounded-lg sm:rounded-xl 
                               px-3 py-2 sm:px-3.5 sm:py-2.5 text-xs sm:text-sm text-gray-900 
                               placeholder-gray-500 focus:outline-none focus:border-orange-500 
                               focus:ring-2 sm:focus:ring-3 focus:ring-orange-200/50 transition-all duration-300 
                ...
                <div className="mb-3 sm:mb-3.5">
                  ...
                  <div className="grid grid-cols-2 gap-2 sm:gap-2.5">
                    {teachingModeOptions.map(({ value, label }) => (
                      <button
                        key={value}
                        type="button"
                        onClick={() => handleCheckboxChange('teachingModes', value)}
                        className={`relative overflow-hidden group flex items-center justify-center p-2 sm:p-2.5 rounded-lg sm:rounded-xl border-2 transition-all duration-300 ${
                          formData.teachingModes.includes(value)
                            ? 'border-orange-500 shadow-sm sm:shadow-md'
                            : 'border-gray-300/80 hover:border-orange-400/60'
                        } ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
                        style={{
                          background: formData.teachingModes.includes(value)
                            ? 'linear-gradient(135deg, rgba(249, 115, 22, 0.08) 0%, rgba(251, 191, 36, 0.08) 100%)'
                            : 'rgba(255,255,255,0.9)'
                        }}
                        disabled={isSubmitting}
                      >
                        <input
                          type="checkbox"
                          className="hidden"
                          checked={formData.teachingModes.includes(value)}
                          onChange={() => {}}
                          disabled={isSubmitting}
                        />
                        ...
                        <span className={`text-xs sm:text-sm font-semibold relative z-10 ${
                          formData.teachingModes.includes(value) ? 'text-gray-900' : 'text-gray-700'
                        }`}>
                          {label}
                        </span>
                ...
                <div className="mb-3 sm:mb-3.5">
                  <label className="block text-xs sm:text-sm font-medium text-gray-800 mb-1.5 sm:mb-1.5 ml-1">
                    <span className="flex items-center gap-1.5 sm:gap-2">
                      <div className="w-4 h-4 sm:w-5 sm:h-5 rounded-md bg-gradient-to-br from-orange-500 to-amber-500 flex items-center justify-center">
                        <School size={10} className="sm:w-3 sm:h-3 text-white" />
                      </div>
                      Affiliated Institution
                      <span className="text-red-500 text-sm sm:text-base">*</span>
                    </span>
                  ...
                  <div className="relative group">
                    <input
                      ...
                      onChange={handleInputChange}
                      placeholder="University, gurukul, or research institute"
                      className="w-full bg-white/90 border-2 border-gray-300/80 rounded-lg sm:rounded-xl 
                               px-3 py-2 sm:px-3.5 sm:py-2.5 text-xs sm:text-sm text-gray-900 
                               placeholder-gray-500 focus:outline-none focus:border-orange-500 
                               focus:ring-2 sm:focus:ring-3 focus:ring-orange-200/50 transition-all duration-300 
                ...
                <div className="mb-3 sm:mb-3.5">
                  ...
                  <div className="relative group">
                    <textarea
                      ...
                      onChange={handleInputChange}
                      rows="4"
                      placeholder="Describe your primary area of research and scholarly focus (minimum 50 characters)..."
                      className="w-full bg-white/90 border-2 border-gray-300/80 rounded-lg sm:rounded-xl 
                               px-3 py-2 sm:px-3.5 sm:py-2.5 text-xs sm:text-sm text-gray-900 
                               placeholder-gray-500 focus:outline-none focus:border-orange-500 
                               focus:ring-2 sm:focus:ring-3 focus:ring-orange-200/50 transition-all duration-300 
                  ...
                  <div className="flex justify-between items-center mt-2">
                    ...
                    <p className={`text-[10px] sm:text-xs ${formData.researchArea.length >= 50 ? 'text-emerald-600 font-medium' : 'text-gray-500'}`}>
                      {formData.researchArea.length} characters (min. 50)
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-2.5 sm:gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] sm:text-xs text-gray-700">
                      ...
                      Publications field is optional. Research area requires minimum 50 characters.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="mt-3 sm:mt-4 flex flex-col items-center">
              ...
              <div className="flex items-center gap-1.5 sm:gap-2 px-2 py-1 sm:px-3 sm:py-1 bg-gradient-to-r from-amber-50 to-orange-50 rounded-full border border-amber-100">
                ...
                <p className="text-[10px] sm:text-xs font-medium text-amber-800 whitespace-nowrap">
                  {isSubmitting ? 'Submitting...' : 'Step 2 of 2 - Scholarly Details Complete'}
                </p>
              </div>
            </div>
            ...
            <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-gray-300/50">
              <div className="flex items-center justify-center gap-2 sm:gap-2.5">
                <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-gradient-to-br from-orange-500/10 to-amber-500/10 flex items-center justify-center">
                  <BookOpen size={10} className="sm:w-3 sm:h-3 text-orange-600" />
                </div>
                <p className="text-center text-[10px] sm:text-xs text-gray-600">
                  Preserve and disseminate ancient Vedic wisdom for future generations
                </p>
              </div>
            </div>
